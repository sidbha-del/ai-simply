const app = {
    ...
    shareOnLinkedIn: function () {
        // Build contextual share message based on current state
        let title = "AI Simply - Learn AI in 2-Minute Lessons";
        let summary = "Master AI concepts with bite-sized, 2-minute chapter lessons. Each learning mode takes only ~10 minutes total. Perfect for busy professionals who want to understand AI without the complexity.";

        // Add context based on current page
        if (this.currentMode && this.currentChapter !== null) {
            const modeData = courseData.modes[this.currentMode];
            const theme = themes[this.currentTheme];
            const chapter = modeData.chapters[this.currentChapter];

            title = `${chapter.title} - AI Simply`;
            summary = `I'm learning about "${chapter.title}" using AI Simply's ${theme.name} persona. ` +
                `Each chapter is just 2 minutes, making complex AI concepts incredibly digestible. ` +
                `Try it yourself - learn AI the simple way!`;
        } else if (this.currentTheme) {
            const theme = themes[this.currentTheme];
            summary = `Learning AI concepts through the ${theme.name} persona on AI Simply. ` +
                `Bite-sized 2-minute lessons make mastering AI effortless. Join me!`;
        }

        const url = window.location.href;
        const linkedinUrl = `https://www.linkedin.com/shareArticle?mini=true&url=${encodeURIComponent(url)}&title=${encodeURIComponent(title)}&summary=${encodeURIComponent(summary)}&source=AI Simply`;
    ...
    shareOnTwitter: function () {
        let shareText = "Just discovered AI Simply! Learn AI concepts with 2-minute chapter lessons. Perfect bite-sized learning! 🚀";

        // Add context if on a specific chapter
        if (this.currentMode && this.currentChapter !== null) {
            const modeData = courseData.modes[this.currentMode];
            const chapter = modeData.chapters[this.currentChapter];
            shareText = `Learning about "${chapter.title}" on AI Simply! 🧠 Each chapter = 2 minutes. Makes AI super digestible. Check it out! 🚀`;
        } else if (this.currentTheme) {
            const theme = themes[this.currentTheme];
            shareText = `Learning AI the simple way with ${theme.name}! 2-minute lessons, zero complexity. 🚀`;
        }
    ...
    copyShareLink: function () {
        ...
        const shareText = `Check out AI Simply - learn AI with 2-minute chapters! ${url}`;
        ...
        if (navigator.clipboard) {
            navigator.clipboard.writeText(shareText).then(() => {
                ...
                btn.innerHTML = `
                    ...
                    <div class="text-left"><div class="font-bold">Copied!</div><div class="text-xs opacity-80">Link copied to clipboard</div></div>
                `;
                setTimeout(() => {
            ...
            alert('Copy this link: ' + shareText);
        }
    }
};
